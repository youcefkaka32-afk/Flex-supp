...
export default function Chatbase() {
  ...
  return (
    <>
      {!open && (
        <button className="chatbase-launch" onClick={() => setOpen(true)} aria-label="Open chat">Chat</button>
      )}
      ...
      {open && (
        <div className="chatbase-iframe-wrap" role="dialog" aria-label="Chat widget">
          ...
          />
          <button className="chatbase-close" onClick={() => setOpen(false)} aria-label="Close chat">×</button>
        </div>
      )}
    </>
  )
