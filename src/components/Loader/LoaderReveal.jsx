...
export default function LoaderReveal({ isFirstLoad, onComplete }) {
  ...
  const [lottieSrc, setLottieSrc] = useState('/lightning.lottie')
  ...
  // Load dotlottie via fetch to ensure our global fetch wrapper is used and
  // to provide a blob URL fallback (avoids potential cross-origin preflight
  // issues when third-party loaders attempt to set custom headers).
  useEffect(() => {
    if (typeof window === 'undefined') return
    let cancelled = false
    ;(async () => {
      try {
        const res = await fetch('/lightning.lottie')
        if (!res.ok) return
        const blob = await res.blob()
        const url = URL.createObjectURL(blob)
        if (!cancelled) setLottieSrc(url)
      } catch (e) {
        // keep default src; fetching may be blocked in some environments
        console.warn('[LoaderReveal] could not fetch lottie blob', e)
      }
    })()
    return () => {
      cancelled = true
      try { if (lottieSrc && lottieSrc.startsWith('blob:')) URL.revokeObjectURL(lottieSrc) } catch (e) {}
    }
  }, [])
