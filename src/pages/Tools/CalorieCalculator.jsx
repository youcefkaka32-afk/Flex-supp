...
Important: Ces estimations sont des points de départ. Ajustez en fonction de vos résultats et consultez un professionnel si nécessaire.`,
    ...
    ar: {
      ...
      activities: {
        '1.2': 'قليل الحركة',
        '1.375': 'نشط قليلاً (1-3 أيام)',
        '1.55': 'نشط معتدل (3-5 أيام)',
        '1.725': 'نشط جداً (6-7 أيام)',
        '1.9': 'نشط للغاية (رياضي)',
      },
      goalLabel: 'الهدف',
      goals: {
        lose: 'فقدان الوزن (-500)',
        maintain: 'الحفاظ على الوزن',
        gain: 'زيادة الوزن (+500)',
      },
      calculate: 'احسب',
      ...
      <div className="tool-content">
        <div className="section-shell">
          ...
          <div className="tool-sidebar">
            ...
            <div className="info-box">
              <h4>{lang === 'ar' ? 'نصيحة' : lang === 'fr' ? 'Conseil' : 'Tip'}</h4>
              <p>
                {lang === 'ar' 
                  ? 'تتبع سعراتك الحرارية لمدة أسبوعين واضبط بناءً على نتائجك. زن نفسك في نفس الوقت كل أسبوع.'
                  : lang === 'fr' 
        ...
        <div className="visual-diagram">
          <h3 className="diagram-title font-display">
            {lang === 'ar' ? 'توزيع المغذيات الكبرى' : lang === 'fr' ? 'Répartition des Macronutriments' : 'Macronutrient Distribution'}
          </h3>
          <div className="diagram-grid">
            <div className="diagram-item">
              <h5>{lang === 'ar' ? 'البروتين' : lang === 'fr' ? 'Protéines' : 'Protein'}</h5>
              <p style={{ fontSize: '32px', fontWeight: '900', color: 'var(--red)', margin: 'var(--space-m) 0' }}>
                25-35%
              </p>
              <p>{lang === 'ar' ? 'بناء العضلات' : lang === 'fr' ? 'Construction musculaire' : 'Muscle building'}</p>
            </div>
            <div className="diagram-item">
              <h5>{lang === 'ar' ? 'الكربوهيدرات' : lang === 'fr' ? 'Glucides' : 'Carbs'}</h5>
              <p style={{ fontSize: '32px', fontWeight: '900', color: 'var(--red)', margin: 'var(--space-m) 0' }}>
                40-50%
              </p>
              <p>{lang === 'ar' ? 'طاقة للتدريب' : lang === 'fr' ? 'Énergie pour l\'entraînement' : 'Energy for training'}</p>
            </div>
            <div className="diagram-item">
              <h5>{lang === 'ar' ? 'الدهون' : lang === 'fr' ? 'Lipides' : 'Fats'}</h5>
              <p style={{ fontSize: '32px', fontWeight: '900', color: 'var(--red)', margin: 'var(--space-m) 0' }}>
                20-30%
              </p>
              <p>{lang === 'ar' ? 'الهرمونات والصحة' : lang === 'fr' ? 'Hormones et santé' : 'Hormones and health'}</p>
            </div>
          </div>
        </div>
